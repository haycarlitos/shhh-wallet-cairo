...
- [`docs/v8-3-smoke-tests.md`](./docs/v8-3-smoke-tests.md) — mainnet smoke-test status. Passed on mainnet: STARK / EIP-191 / ED25519 / WebAuthn / **raw P-256** OEs, V8.4 deploy, governance propose, **session-key spending caps** (Test 14 — over-cap reverts on-chain, in-cap succeeds), and the **guardian-recovery carve-out** (Test 13 — guardian initiates, guardian's non-recovery OE reverts `'SHHH: signer not an owner'`, owner cancels). Remaining (threshold, recovery finalize, and signer kinds secp256k1 / EIP-712 / JWT-ES256 / BLS) are snforge-only.
