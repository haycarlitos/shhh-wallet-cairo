# Shhh V8.3 — SDK Integration Spec (Internal)

> **Audience**: Chipi Pay engineering, Cifra integration, Shhh frontend, anyone wiring V8.3 into a TypeScript / wallet codebase.
>
> **Status**: V8.3 audit-closed against [2026-05-10 review](../audits/2026-05-10-claude-opus-v8-2-review.md), declared on Starknet mainnet 2026-05-11 (class hash `0x03bc539295abd3e59bd9ea799d12fe3331748d484bc77bff45b302b1636a87d9`). 242/242 snforge tests passing. One smoke-test deployment + signed OE confirmed end-to-end on mainnet 2026-05-10 against the V8.1 class — the dispatcher path is byte-for-byte identical from V8.1 → V8.3, only the validation entry points and the `inside_verifier` flag wrapping changed, so the end-to-end OE proof carries.
>
> **Repo references**:
>   - Cairo source: `haycarlitos/shhh-wallet-cairo`, branch `v8-robust`, V8.3 merged at commit `af45e95`.
>   - Smoke-test reference deployment: [`0xa2220b05b5d16f52c4ce179e5386bd4b23dc5f4b7a1e0b51c579dbd9129b31`](https://voyager.online/contract/0x00a2220b05b5d16f52c4ce179e5386bd4b23dc5f4b7a1e0b51c579dbd9129b31) (V8.1 class — V8.3 deploys land at fresh addresses since the class hash differs).
>   - V8.3 ShhhAccount declare tx: [`0x06653accbeb2f9628488d64d4d616db6939c028dc77eb275eb78229b3d16712a`](https://voyager.online/tx/0x06653accbeb2f9628488d64d4d616db6939c028dc77eb275eb78229b3d16712a).

---

## 1. What V8.3 is, in one paragraph

V8.3 is one Starknet account class (`ShhhAccount`) that authenticates owners signing under any of ten cryptographic primitives — STARK, Ed25519, raw secp256k1, EIP-191 `personal_sign`, EIP-712 typed data, raw P-256, WebAuthn P-256, JWT-ES256 single-tenant Apple, JWT-ES256 sub-bound multi-tenant Apple, and BLS12-381 min-sig-size. Signature verification dispatches via `library_call_syscall` to a separately-declared verifier class. New signing kinds land by declaring a new verifier class and registering it through governance — no account redeployment. Multi-owner, weighted threshold, timelocked governance, 7-day guardian recovery, and SNIP-163 session keys are all built in. Outside execution (SNIP-9 V2) is the sole execution path; `__validate__` always reverts. The `ISigner` trait gained a third method (`validate_pubkey`) in V8.2 so the account can validate per-kind shape + curve membership at owner registration via library_call; V8.3 wires it into all four owner-mutation entry points (`execute_add_owner`, `execute_rotate_owner`, `finalize_recovery`, `bootstrap_from_sessions`) with the `inside_verifier` reentrancy flag raised around each call.

---

## 2. Class hashes (V8.3, mainnet — 2026-05-11)
...
|---|---|---|
| `ShhhAccount` (V8.3) | `0x03bc539295abd3e59bd9ea799d12fe3331748d484bc77bff45b302b1636a87d9` | V8.3 account — dispatcher + multi-owner + governance + recovery + sessions |
| `StarkVerifier` (V8.2) | `0x00d09209b2da9d49fc805ba26380ba4ce25aa641116c10eb178e1051a71dbf68` | STARK ECDSA |
| `Ed25519Verifier` (V8.2) | `0x030a7dfc03e59cef6e41699e734abd2df53ce393a052221c02c6e07665949f74` | Ed25519 (Phantom / Solana) |
| `Secp256k1Verifier` (V8.2) | `0x03e81667a46bd5287e09a9600fa98d28fdc477735f2689f5f4e8e95f37b67b74` | Raw secp256k1 ECDSA |
| `EIP191Secp256k1Verifier` (V8.2) | `0x03a75997862059c36cb8e204fb3027eb6d1fdf933488d42c2db4528118d084e6` | EIP-191 `personal_sign` (MetaMask, Rabby) |
| `EIP712Secp256k1Verifier` (V8.2) | `0x072a3f77e8c28bfea2ade91ec3fb83b6290169d1ed8c1b2396704231841c6474` | EIP-712 typed data (`eth_signTypedData_v4`) |
| `P256Verifier` (V8.2) | `0x01b600709af54c8838e5f18ddad3a26feeb47cb124c239f55a0f1b7a780e2d8a` | Raw P-256 ECDSA |
| `WebAuthnP256Verifier` (V8.2) | `0x074f6efd2af9025cd8cab41a4565bc73b6ef097214c31352838fcdbac0a44657` | Apple passkeys / Touch ID / Face ID / WebAuthn |
| `JwtES256AppleVerifier` (V8.2) | `0x002efce875fa3e73e04d825d8ebade53e188cc995dfe0c55a6a2f7fa6c59f497` | Sign in with Apple, single-tenant |
| `JwtES256AppleSubVerifier` (V8.2) | `0x06b67762218a25fdd28e25b063480893a5cef9cdeecbc663e32d444d5734c471` | Sign in with Apple, multi-tenant (sub-bound) |
| `Bls12_381MinSigVerifier` (V8.2) | `0x02623721e74a9ad3e0ba639065f5631a09bf900913de6ab21ea6984973cd2cd1` | BLS12-381 min-sig (drand DST) |

Deprecated (do **not** use for new deploys):

| Contract | Class hash | Why |
|---|---|---|
| `ShhhAccount` V8.2 | `0x02a0b719…1a062` | Pre-2026-05-10-audit; missing finalize_recovery / bootstrap validate_pubkey calls (H-1 + M-2) and asymmetric inside_verifier flag on validate path (M-1). |
| `ShhhAccount` V8.1 | `0x01e7f69e…f363b5` | Pre-V8.2; `ISigner` trait has 2 methods, missing `validate_pubkey` (M-1 partial only). |
| `ShhhAccount` V8.0 | `0x01d6e475…ae3` | Vulnerable to C-1 (guardian role bypass) and H-1 (bootstrap front-run); pre-V8.1 audit. |
| `*Verifier` V8.1 (every kind) | various | Pre-V8.2 trait shape (2 methods, no `validate_pubkey`). Incompatible with V8.2/V8.3 ShhhAccount via library_call — would `ENTRYPOINT_NOT_FOUND` on validate_pubkey dispatch. Kept declared only for legacy recognition. |

**Critical**: Existing V8.0 / V8.1 / V8.2 account instances **cannot self-upgrade** to V8.3 — V8 deliberately ships without an `upgrade` selector. The only path from a deprecated class to V8.3 is to deploy a fresh V8.3 account at a new address (the salt changes because the class hash is bound into the address derivation) and migrate assets manually. The legacy V8.0/V8.1 smoke-test instance at `0xa222…29b31` is intentionally not upgraded.
...
// V8.3 ShhhAccount class hash (declared 2026-05-11). The class hash
// includes the validate_pubkey wiring across all four owner-mutation
// entry points (audit 2026-05-10 closure).
export const V8_SHHH_ACCOUNT_CLASS_HASH =
  "0x03bc539295abd3e59bd9ea799d12fe3331748d484bc77bff45b302b1636a87d9";

// V8.2 verifier class hashes — unchanged across V8.2 → V8.3 because
// the trait shape did not change (V8.3 was an account-only redeclare).
...
async function submitOE(args: {
  account: string;            // V8.3 ShhhAccount address
...
The Cairo repo's test suite exercises every kind via these fixtures. **242/242 tests passing** as of the V8.3 audit-closed merge (commit `af45e95` on `v8-robust`, declared 2026-05-11). The +24 tests vs. V8.1 cover the V8.2 trait-shape change (`validate_pubkey` per-kind regressions) and the V8.3 closeout (M-1 symmetric reentrancy, M-2 bootstrap validate, M-3 evil-verifier negative panics, H-1 finalize_recovery validate).
...
4. **`ShhhMigrationFromSessions` class.** The atomic-upgrade path for existing chipi-pay/sessions-smart-contract users (one OE: `[upgrade(MIGRATION_CLASS), bootstrap_from_sessions(...)]`). Not declared. Existing sessions users today must deploy a fresh V8.3 account and transfer assets; no in-place migration. Gating step before V8 fully replaces V7.5/sessions for legacy users.
5. **In-place upgrade from V8.0 / V8.1 / V8.2 → V8.3.** Not supported by design — V8 ships without an `upgrade` selector. Holders of deprecated `ShhhAccount` instances must redeploy at a fresh address (the class hash is bound into the deterministic salt, so the address changes) and migrate funds manually. The smoke-test V8.1 instance at `0xa222…29b31` is intentionally left in place as a regression reference.
6. **H-2 dedicated bypass-attempt fixture** (Apple JWT with `sub_offset` pointing into `name.firstName`). The inline anchor check in the JWT-Apple-sub verifier closes the bug; a dedicated regression test is the V8.3 follow-up tracked in `audits/2026-05-10-claude-opus-v8-2-review.md` L-1.
7. **Phase 13 external audit** (Omar/Codex or Zellic/Nethermind/OZ). V8.3 is audit-ready, audited internally only. The 2026-04-20 Codex pass, the 2026-04-13 Nethermind AuditAgent scan, and the three internal self-reviews (2026-05-07 / 2026-05-10 / 2026-05-11) are lower-tier than a full institutional audit. SNIP-108 submission to `starknet-io/SNIPs` does not require external audit completion, but mainnet promotion to "production" labeling does.

---

## 19. M-1 history — V8.1 partial → V8.2 full → V8.3 wiring

> **This section is historical.** All work described here is **shipped** in V8.3. It is kept in the SDK doc because (a) the V8.1 smoke-test deployment at `0xa222…29b31` still runs the partial path and integrators need to know why it differs, and (b) the per-kind `validate_pubkey` semantics here are what the SDK must build envelopes against.

**What V8.1 shipped (the partial fix, deprecated)**:
- `_assert_pubkey_shape(kind, pubkey)` in `account.cairo` checked `pubkey.len() == expected_len` per kind tag at every registration site. Unknown kind → revert.
- Caught ~80% of registration footguns: wrong-shape garbage like "16 felts of `1` for BLS", "4 felts where 5 are needed for sub-bound JWT", etc.

**What V8.1 did NOT catch (the gap V8.2 closed)**:
- A 16-felt BLS pubkey that has correct shape but is **not in the BLS12-381 G2 r-torsion subgroup** (cofactor-style attack). Such a pubkey passes the length check, lands in `owners`, and every subsequent verify involving that owner panics inside `Bls12_381MinSigVerifier::verify`.
- For `secp256k1` / `P256` / `WEBAUTHN_P256`, a `(x, y)` off the curve. The respective verifier returns `false` gracefully — not a DoS, but a permanent dead owner_id.

**V8.1 impact** (audit-rated Medium, now closed):
- No fund-theft path.
- The harm was **multisig DoS**: any threshold OE that included the poisoned `owner_id` reverted. Legitimate signers couldn't satisfy the threshold without going through the timelocked `remove_owner` flow first.

**What V8.2 added (the full ISigner closure, declared 2026-05-10)**:
...
4. **Redeclare all 11 classes** (V8.2 ShhhAccount + 10 verifier classes). Class hashes change because Sierra changes when the trait shape changes. ✅ Shipped 2026-05-10.

5. **Update SDK constants** to V8.2 hashes. ✅ Section 4 of this doc tracks them.

6. **Update tests** to cover per-kind validate_pubkey regressions. ✅ V8.2 added 21 new tests, V8.3 added 3 more (M-1 / M-2 / M-3 negatives). Final count: 242/242.

**Realized V8.2 declare cost (2026-05-10)**: ~210 STRK across the 11 redeclares — within the original estimate of ~204 STRK.

**V8.3 follow-on (declared 2026-05-11, account-class only)**:
- H-1: `finalize_recovery` now calls `validate_pubkey` before committing the new owner record.
- M-1 symmetric: the `inside_verifier` flag is raised around `validate_pubkey` library_calls as well as `verify` library_calls — symmetric reentrancy guard.
- M-2: `bootstrap_from_sessions` now calls `validate_pubkey` on the migrated primary owner before committing it.
- M-3: the `EvilReentrantVerifier` / `EvilReturnTrueVerifier` / `EvilPanicVerifier` test helpers ship in `src/test_helpers/evil_verifier.cairo` and back three executable negative regressions (`tests/audit_v8_3.cairo`) — V8.2 closed the M-3 finding semantically but only added "documented in code", V8.3 makes it executable.
- Verifier class hashes unchanged from V8.2 (only the account-class binary changed). V8.3 redeclare cost: ~46 STRK for the single ShhhAccount class.

**Why we shipped the partial first**: the audit was clear that M-1 is **Medium severity, no fund-theft path**. Critical/High findings (C-1, H-1, H-2, H-3) were the gating items for V8.1; M-1 partial closed ~80% of the surface. Full M-1 became the V8.2 batch (one redeclare cycle rather than per-finding) and the V8.3 wiring closeout caught the integration sites V8.2 missed.
...
- [ ] Run a mainnet smoke test: deploy one V8.3 account with each supported kind as primary, sign one OE per kind, confirm in a block.
